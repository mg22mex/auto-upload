...
| **voice_gateway** | `src/voice_gateway/` | FastAPI: voice/Meta webhooks + standalone `vapi_bridge` (`POST /vapi/inventory` with 2s Odoo timeout + 15m TTL cache, financing, trade-in, lead). Pipeline: intent/STT → quote → Odoo → PDF. |
