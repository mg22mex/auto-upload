...
| **voice_gateway** | `src/voice_gateway/` | FastAPI: voice/Meta webhooks + standalone `vapi_bridge` (`POST /vapi/inventory` with 2s Odoo timeout + 15m TTL cache, financing, trade-in, lead). Pipeline: intent/STT → quote → Odoo → PDF. Public tool URL via user systemd `cloudflared-vapi-bridge` (`--protocol http2 --edge-ip-version 4`; named tunnel steps in `deploy/cloudflared-named-tunnel.md`). |
